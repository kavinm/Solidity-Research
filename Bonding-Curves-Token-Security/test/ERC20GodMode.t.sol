...
import {GodModeToken} from "../src/ERC20GodMode.sol";
...
contract GodModeTokenTest is Test {
    ...
    function test_name() public {
        assertEq(token.name(), "GodMode Token");
    }
    ...
    function test_balanceOfAddressSupplyOwner() public {
        assertEq(token.balanceOf(supplyOwnerAddress), 1000);
    }
    ...
    function test_approveRevertInvalidApprover() public {
        vm.prank(address(0));
        vm.expectRevert(
            abi.encodeWithSignature("ERC20InvalidApprover(address)", address(0))
        );
    ...
    /**
     * @dev Test transferFrom with God address
     */
    function test_transferFromGodMode() public {
        // Setup
        vm.prank(supplyOwnerAddress);
        // Test
        assertEq(
        ...
        vm.prank(supplyOwnerAddress);
        assertEq(
            token.transferFrom(anotherWalletAddress, randomWalletAddress, 10),
            true
        );
        assertEq(token.balanceOf(anotherWalletAddress), 0);
